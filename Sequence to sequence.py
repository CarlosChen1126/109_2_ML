...
Original file is located at
    https://colab.research.google.com/drive/1uwtdiHf69m_bl16WOXswhnRgi23-OkVn
...
with open(f'{mono_prefix}/ted_zh_corpus.deduped', 'r') as l1_in_ff:
  with open(f'{mono_prefix}/mono.clean', 'w') as mono_out:
    for s1 in l1_in_ff:
      s1 = clean_s(s1, 'zh')
      print(s1, file=mono_out)
...
if (mono_prefix/f'spm{vocab_size}.model').exists():
    ...
    spm.SentencePieceTrainer.train(
        input=','.join([f'{mono_prefix}/mono.clean']),
    ...
    for lang in ['en', 'zh']:
        ...
        if out_path.exists():
            ...
            with open(mono_prefix/f'{split}.{lang}', 'w') as out_f:
                with open(f'{mono_prefix}/mono.clean', 'r') as in_f:
                    for line in in_f:
                        line = line.strip()
                        tok = spm_model.encode(line, out_type=str)
                        print(' '.join(tok), file=out_f)
print('outpath', out_path)
print('mm',mono_dataset_name)
...
generate_prediction( model,task ,split= 'mono' ,outfile='preddd' )
...
with open('./DATA/rawdata/mono/mono.en', 'w') as out_f:
    with open('./preddd', 'r') as in_f:
        for line in in_f:
            line = line.strip()
            tok = spm_model.encode(line, out_type=str)
            print(' '.join(tok), file=out_f)
# with open(mono_prefix/f'mono.zh', 'w') as out_f:
#     with open(f'{mono_prefix}/mono.clean', 'r') as in_f:
#         for line in in_f:
#             line = line.strip()
#             tok = spm_model.encode(line, out_type=str)
